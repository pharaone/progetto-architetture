...
class MenuService:
    ...
    async def create_menu_item(self, menu_item: MenuItem) -> bool:
        """
        Aggiunge un nuovo piatto al menu di una cucina.
        """

        print(f"DEBUG Service: Metodo create_menu_item ricevuto: {menu_item.dish_id}")
        # Controlla se il piatto esiste già per evitare la sovrascrittura accidentale
        if await self.get_menu_item(menu_item.dish_id):
        ...
        try:
            await asyncio.to_thread(self._menu_repo.create_menu_item, menu_item)
            return True
    ...
    async def get_menu_item(self, dish_id: uuid.UUID) -> Optional[MenuItem]:
    ...
    async def is_dish_available(self, dish_id: uuid.UUID) -> bool:
        ...
        """
        item = await self.get_menu_item(dish_id)
    ...
    async def update_menu_item(self, menu_item: MenuItem) -> bool:
        ...
        if not await self.get_menu_item(menu_item.dish_id):
    ...
    async def commit_order_dish(self, dish_id: uuid.UUID) -> bool:
        """
        ...
        result_code = await asyncio.to_thread(self._menu_repo.atomic_decrement_quantity, dish_id)
    ...
    async def restock_item(self, dish_id: uuid.UUID, amount: int = 1) -> bool:
        """
        Incrementa la quantità di un piatto in modo atomico e non bloccante.
        """
        # Eseguiamo la chiamata bloccante del repository in un thread separato.
        result_code = await asyncio.to_thread(self._menu_repo.atomic_increment_quantity, dish_id, amount)
